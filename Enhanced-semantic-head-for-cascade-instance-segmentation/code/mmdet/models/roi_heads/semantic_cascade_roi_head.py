...
class SCascadeRoIHead(CascadeRoIHead):
    def __init__(self,
                 num_stages,
                 stage_loss_weights,
        ...
        super(SCascadeRoIHead, self).__init__(
    ...
    def forward_dummy(self, x, proposals):
        ...
        if self.with_semantic:
            _, semantic_feat,scores = self.semantic_head(x)
        else:
            semantic_feat = None
        ...
        if self.with_mask:
            mask_rois = rois[:100]
            for i in range(self.num_stages):
                mask_results = self._mask_forward(i, x, mask_rois,semantic_feat=semantic_feat)
    ...
    def _mask_forward(self, stage, x, rois,semantic_feat = None):
    ...
    def _mask_forward_train(self,
        ...
        mask_results = self._mask_forward(stage, x, pos_rois, semantic_feat = semantic_feat)
    ...
    def forward_train(self,
        ...
        losses = dict()
        if self.with_semantic:
            semantic_pred, semantic_feat, scores = self.semantic_head(x)
            loss_seg,loss_cls = self.semantic_head.loss(semantic_pred, gt_semantic_seg, scores, gt_labels)
            losses['loss_semantic_seg'] = loss_seg
            losses['loss_semantic_cls'] = loss_cls
        else:
            semantic_feat = None
        ...
        for i in range(self.num_stages):
            ...
            bbox_results = self._bbox_forward_train(i, x, sampling_results,
                                                    gt_bboxes, gt_labels,
                                                    rcnn_train_cfg,semantic_feat = semantic_feat)
            ...
            if self.with_mask:
                mask_results = self._mask_forward_train(
                    ...
                    bbox_results['bbox_feats'],semantic_feat = semantic_feat)
    ...
    def aug_test(self, features, proposal_list, img_metas, rescale=False):
        ...
        aug_bboxes = []
        aug_scores = []
        for x, img_meta,semantic in zip(features, img_metas,semantic_feats):
        ...
        if self.with_mask:
            if det_bboxes.shape[0] == 0:
                ...
                for x, img_meta,semantic in zip(features, img_metas,semantic_feats):
                    ...
                    mask_rois = bbox2roi([_bboxes])
                    for i in range(self.num_stages):
                        mask_results = self._mask_forward(i, x, mask_rois,semantic_feat=semantic)
                        aug_masks.append(
